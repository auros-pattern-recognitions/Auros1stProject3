...
namespace Auros1stProject3
{
    class Program
    {
        static void Main(string[] args)
        {
            ...
            // 프레넬 반사계수를 담을 배열.
            Complex rp = new Complex(0, 0),
                    rs = new Complex(0, 0);
            // 프레넬 투과계수를 담을 배열.
            Complex tp = new Complex(0, 0),
                    ts = new Complex(0, 0);
            ...
            const int LastLayer = 1000;     // 박막 층수. (pair = 층수 / 2)
            ...
            /*for (int i = 0; i < LenData; i++)
                WriteLine($"alpha_cal: {alpha_cal[i]} \t beta_cal: {beta_cal[i]}");*/
            ...
            #region 파일 쓰기.
            // 파일 쓰기.
            using (StreamWriter NewSpectrumOutputFile = new StreamWriter("500pairs.dat"))
            {
                // 컬럼 명 쓰기.
